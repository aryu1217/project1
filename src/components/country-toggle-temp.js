...
export default function CountryToggle() {
  ...
  const selected = searchParams.get("country") || "domestic";
  ...
  return (
    <div className="flex mx-auto w-[350px] bg-[#FFFFFF]/10 rounded-full p-1">
      <button
        onClick={() => handleClick("domestic")}
        className={`flex-1 px-4 py-1 rounded-full transition-all ${
          selected === "domestic" ? "bg-[#141517] text-white" : "text-[#FFFFFF]"
        }`}
      >
        국내
      </button>
